...
from train_ccgan import train_ccgan_with_kd, sample_ccgan_given_labels
...
if args.GAN_arch == "SNGAN":
    netG_teacher = sngan_generator(nz=args.dim_gan, dim_embed=args.dim_embed)
else:
...
netG_student = sngan_generator_student(
    nz=args.dim_gan,
...
netD = sngan_discriminator(
    dim_embed=args.dim_embed,
    disc_ch=32  # 比教师的64小
)

Filename_Student = save_models_folder + '/ckpt_student_niter_{}.pth'.format(args.niters_gan)
if os.path.isfile(Filename_Student):
    print("\n Loading pre-trained student model >>>")
    checkpoint_student = torch.load(Filename_Student)
    netG_student.load_state_dict(checkpoint_student['netG_student_state_dict'], strict=False)  # 添加strict参数
    netD.load_state_dict(checkpoint_student['netD_student_state_dict'], strict=False)
    print("Student model loaded from:", Filename_Student)
else:
    # 原有训练流程保持不变
